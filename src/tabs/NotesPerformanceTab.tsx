...
import { NoteFormatOption, SubstackNote } from '../types';
...
export const NotesPerformanceTab: React.FC = () => {
  ...
  const [activeNote, setActiveNote] = useState<SubstackNote | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Notes Performance Table with Sorting, Pagination & Detail Modal */}
          ...
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-100 text-slate-600 font-bold uppercase text-[11px] tracking-wider border-b border-slate-200 select-none">
              <tr>
                ...
                <th className="py-3.5 px-4 text-right">SMM Diagnostic</th>
              </tr>
            </thead>
      ...
      {/* Note Detail & SMM Blueprint Modal */}
      {activeNote && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="glass-panel max-w-xl w-full rounded-2xl p-6 border border-slate-300 bg-white shadow-2xl relative animate-in fade-in zoom-in duration-200">
            <button
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <span className="text-[11px] font-bold text-amber-600 uppercase tracking-widest">
                  Note SMM Hook & Teaser Diagnostic
                </span>
                <h3 className="text-sm font-bold text-slate-900 leading-snug">
                  Published: {activeNote.publishDate} • Format: {activeNote.format}
                </h3>
              </div>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 mb-4 text-xs">
              <span className="text-[10px] font-bold uppercase text-slate-500 block mb-1">Full Note Content:</span>
              <p className="text-slate-900 font-serif leading-relaxed italic">"{activeNote.hook}"</p>
            </div>

            <div className="grid grid-cols-4 gap-2.5 p-3 rounded-xl bg-slate-100/70 border border-slate-200 mb-4 text-center text-xs">
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Views</span>
                <strong className="text-slate-900 font-mono">{activeNote.impressions.toLocaleString()}</strong>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Likes</span>
                <strong className="text-rose-600 font-mono">{activeNote.likes}</strong>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Restacks</span>
                <strong className="text-emerald-700 font-mono">{activeNote.restacks}</strong>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Profile Clicks</span>
                <strong className="text-amber-600 font-mono">{activeNote.profileClicks}</strong>
              </div>
            </div>

            {/* Hook Analysis */}
            <div className="p-3.5 rounded-xl bg-blue-50/60 border border-blue-200 text-xs mb-4">
              <strong className="text-blue-900 block font-display text-[10px] mb-1 uppercase tracking-wider">
                🔍 Hook Strength & Readership Diagnosis:
              </strong>
              <p className="text-blue-950 leading-relaxed font-medium">
                {activeNote.hookAnalysis || 'Strong opening hook driving steady profile clicks and reader interest.'}
              </p>
            </div>

            {/* Future Note Action */}
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-xs">
              <div className="flex items-center gap-1.5 text-emerald-900 font-bold font-display mb-1.5">
                <Sparkles className="w-4 h-4 text-emerald-600" />
                <span>🚀 Future Note Actionable Blueprint:</span>
              </div>
              <p className="text-emerald-950 leading-relaxed font-semibold">
                {activeNote.futureNoteAction || 'Pair this format with a key analytical chart within 2 hours of publication to lift quote restacks.'}
              </p>
            </div>
          </div>
        </div>
